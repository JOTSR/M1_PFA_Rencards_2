...
for (const button in menu) {
    menu[button].addEventListener('click', () => {
        for (const item in menu) {
            if ((item !== button) && !$(`#modal-${item}`).classList.contains('modal-hidden')) {
...
$('#controls-carte').addEventListener('click', () => {
...
for (const child of $('#contact_form').children) {
    ...
    if (child.tagName === 'TEXTAREA') {
        child.innerText = localStorage.getItem(`contact_form_${child.name}`) ?? ''
    }
...
root.style.setProperty('--tilt', `-${4 * Math.exp(- (((window.innerWidth - 450) / 450) ** 2))}deg`)
